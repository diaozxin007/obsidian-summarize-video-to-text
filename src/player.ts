 ...
 *
 * postMessage 协议就是 YouTube IFrame API 底层用的那套:先发 {event:"listening"}
...
import { embedUrl, parseVideoBlock, parseYoutubeLink } from "./video";
...
export function renderVideoBlock(source: string, el: HTMLElement): void {
  ...
  const wrap = el.createDiv({ cls: "svt-video" });
  const iframe = wrap.createEl("iframe");
  iframe.src = embedUrl(block.id);
...
function post(iframe: HTMLIFrameElement, msg: Record<string, unknown>): void {
...
export function handleDocumentClick(e: MouseEvent): void {
  ...
  if (!a) return;
  const link = parseYoutubeLink(a.getAttribute("href") ?? "");
  ...
  return Prec.highest(
    EditorView.domEventHandlers({
      click(e, view) {
        ...
        const link = parseYoutubeLink(href);
